...
export function usePrintTemplates(sheetKey: SheetKey): UsePrintTemplatesResult {
  ...
  const create = useCallback(async (input: PrintTemplatePersistInput) => {
    const created = await api<PrintTemplate>("/api/v1/print-templates", {
      method: "POST",
  ...
  const update = useCallback(async (id: string, patch: PrintTemplateUpdateInput) => {
    const updated = await api<PrintTemplate>(`/api/v1/print-templates/${id}`, {
      method: "PATCH",
  ...
  const remove = useCallback(async (id: string) => {
    await api<{ deleted: boolean; id: string }>(`/api/v1/print-templates/${id}`, {
      method: "DELETE"
    });
