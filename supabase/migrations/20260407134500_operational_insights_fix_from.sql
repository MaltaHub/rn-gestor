-- Corrige FROM faltante na view operacional de anúncios (versão grupo+preço)
...
with base as (
  ...
  left join public.repetidos as r on r.carro_id = a.carro_id
)
, chosen_reference as (
...
, computed as (
  ...
  left join ads_per_group_price as gpp on gpp.grupo_id is not distinct from b.grupo_id and gpp.preco is not distinct from b.preco_carro_atual
)
...
select
  ...
  case
    when wv.has_group_duplicate_ads then 'Mais de um veiculo deste grupo esta anunciado (mesmo preço); mantenha apenas o representativo.'
