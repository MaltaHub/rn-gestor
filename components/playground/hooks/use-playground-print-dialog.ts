...
export type PlaygroundPrintOptions = {
...
function hasVisibleRowsInRange(page: PlaygroundPage, range: PlaygroundSelection) {
  ...
  for (let row = normalized.startRow; row <= normalized.endRow; row += 1) {
    if (!isRowHidden(page, row)) return true;
  }
  return false;
}
...
function hasVisibleColumnsInRange(page: PlaygroundPage, range: PlaygroundSelection) {
  ...
  for (let col = normalized.startCol; col <= normalized.endCol; col += 1) {
    if (!isColumnHidden(page, col)) return true;
  }
  return false;
}

/**
 * Impressao do playground: gera o documento e abre DIRETO em outra aba (sem
 * simulador/preview). As opcoes visuais podem vir de um template salvo.
 */
export function usePlaygroundPrint({
  activePage,
  buildPrintDocument,
  onError,
  printablePage,
  selection,
  workbook
}: UsePlaygroundPrintParams) {
  const hasSelection = Boolean(selection);

  const printNow = useCallback(
    (options: PlaygroundPrintOptions) => {
      ...
      const range = options.scope === "selection" ? selectionRange : pageRange;

      if (!range) {
        onError(options.scope === "selection" ? "Selecione uma area antes de imprimir." : "Nao ha dados para imprimir nesta pagina.");
        return;
      }
      ...
      if (!popup) {
        onError("Nao foi possivel abrir a janela de impressao. Permita pop-ups para este site.");
        return;
      }
      ...
      popup.document.write(
        buildPrintDocument({
          page: printPage,
          range,
          title: options.title?.trim() || activePage.name,
          showGridLines: options.showGridLines,
          showSheetIndexes: options.showSheetIndexes,
    ...
    [activePage, buildPrintDocument, onError, printablePage, selection, workbook]
  );

  return { printNow, hasSelection };
}
