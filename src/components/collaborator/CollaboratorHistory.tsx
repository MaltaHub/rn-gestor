...
import { History, ClockIcon, AlertCircle, Filter, Car } from "lucide-react";
import { format } from "date-fns";
import { getCollaboratorHistory } from "@/services/vehicleService";
...
// Interface matching the vehicle_history_with_user view
...
export const CollaboratorHistory: React.FC<CollaboratorHistoryProps> = ({ collaboratorId }) => {
  ...
  useEffect(() => {
    const loadHistory = async () => {
      ...
      try {
        const historyData = await getCollaboratorHistory(collaboratorId);
        setHistory(historyData as HistoryItem[]);
      } catch (err) {
        console.error("Error loading collaborator history:", err);
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-vehicleApp-darkGray">
                        em{" "}
                        <span 
                          className="font-semibold cursor-pointer hover:text-vehicleApp-red hover:underline flex items-center"
